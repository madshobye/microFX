...
| Layer compositor/IPU | Portable retained layer descriptors and the i.MX6 capability/probe map are isolated and tested not to enter the normal image. The bounded C/JavaScript prototype maps ordered membership, logical dimensions and origin, blend, opacity, effects, and independent pixel density into planner requests. The planner preserves back-to-front ordering by allowing native planes only for a contiguous topmost layer suffix; a later GLES effect or plane exhaustion safely flattens all lower layers instead of reordering them. A fixture-tested probe evaluator extracts plane type, RGB formats, alpha and z-position, and refuses to recommend a native experiment until explicit scaling, DMA-BUF, atomic-commit and page-flip evidence all pass. | Capture and evaluate the target plane inventory, then run the scaled-buffer/native-overlay experiment before embedding the facade or implementing a backend. |
