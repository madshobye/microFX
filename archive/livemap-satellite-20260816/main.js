...
const map = scene.add(earth.day);
scene.add(earth.night);
earth.hide();
const earthView = fx.texture(earth.day)
...
// Added after the map's retained objects so this single interface element is
// drawn over the warming full-HD background cache.
const loadingScene = fx.scenes.add(fx.scene({ name: "loading" }));
const loadingText = loadingScene.add(
  fx.text("LOADING 0 / 2", 805, 515, 24, 0x777777ff).antialias(false));

function update(time, delta) {
  clockTime = time;
  const startup = startupAssets.update();
  if (startup.sourcesReady) earthView.show();
  else earthView.hide();
  if (!startup.ready) {
    const dots = ".".repeat(Math.floor(time * 3) % 4);
    loadingText.text(`LOADING ${startup.requiredReady} / ${startup.requiredTotal}${dots}`);
    loadingScene.show();
    updateSun(time);
    return;
  }
