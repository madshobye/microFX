...
# Fast development workflow: engine, raylib backend, shaders and embedded JS.
# It never runs Buildroot clean and never creates a Linux root image.
SOURCE_ARCHIVE=$(mktemp /tmp/microfx-graphics-source.XXXXXX)
...
COPYFILE_DISABLE=1 COPY_EXTENDED_ATTRIBUTES_DISABLE=1 tar -C "$REPO_DIR" \
  ...
  -cf "$SOURCE_ARCHIVE" .
limactl copy --backend=scp "$SOURCE_ARCHIVE" "$VM_NAME:/tmp/microfx-graphics-source.tar"
...
limactl shell "$VM_NAME" -- sh -lc "
  ...
  SRC=\"\$(mktemp -d \"\$HOME/microfx-graphics-src.XXXXXX\")\"
  trap 'rm -rf \"\$SRC\" /tmp/microfx-graphics-source.tar' EXIT
  tar -xf /tmp/microfx-graphics-source.tar -C \"\$SRC\"
  ...
  make -C \"\$BR\" O=\"\$OUT\" raylib-drm-dirclean microfx-demo-dirclean
  make -C \"\$BR\" O=\"\$OUT\" -j\"\$(nproc)\" raylib-drm microfx-demo
