...
(cd "$WORK" && shasum -a 256 canvas-demo libraylib.so.550 libqrencode.so.4 libcurl.so.4 icosahedron.obj main.js ./*.vs ./*.fs version >manifest.sha256)
...
scp -O -i "$KEY" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new \
  ...
  "$WORK/canvas-demo" "$WORK/libraylib.so.550" "$WORK/libqrencode.so.4" "$WORK/libcurl.so.4" "$WORK/icosahedron.obj" "$WORK/main.js" "$WORK"/*.vs "$WORK"/*.fs \
